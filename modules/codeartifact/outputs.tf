...
output "npm_repository_endpoint" { value = aws_codeartifact_repository.npm_store.external_connections[0].external_connection_status == "AVAILABLE" ? "https://${aws_codeartifact_domain.main.domain}-${data.aws_caller_identity.current.account_id}.d.codeartifact.${data.aws_region.current.name}.amazonaws.com/npm/${aws_codeartifact_repository.npm_store.repository}/" : "" }
