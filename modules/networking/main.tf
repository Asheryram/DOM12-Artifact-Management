...
# Private route table — no default route; VPC endpoints handle AWS API traffic
...
# Allows ECS tasks to reach AWS service endpoints via HTTPS
resource "aws_security_group" "vpc_endpoints" {
  name        = "${var.project_name}-vpce-sg"
  description = "HTTPS ingress from ECS tasks to VPC Interface endpoints"
  vpc_id      = aws_vpc.main.id
...
# ── VPC endpoints ─────────────────────────────────────────────────────────────
# Replace NAT gateway: ECS tasks in private subnets reach AWS APIs without
# ever leaving the VPC or needing internet access.
...
resource "aws_vpc_endpoint" "secretsmanager" {
  ...
  tags = merge(local.common_tags, { Name = "${var.project_name}-vpce-secretsmanager" })
}
...
resource "aws_vpc_endpoint" "ecr_dkr" {
  ...
  security_group_ids  = [aws_security_group.vpc_endpoints.id]
  private_dns_enabled = true
  tags = merge(local.common_tags, { Name = "${var.project_name}-vpce-ecr-dkr" })
}
...
resource "aws_vpc_endpoint" "ecr_api" {
  ...
  security_group_ids  = [aws_security_group.vpc_endpoints.id]
  private_dns_enabled = true
  tags = merge(local.common_tags, { Name = "${var.project_name}-vpce-ecr-api" })
}
...
resource "aws_vpc_endpoint" "logs" {
  ...
  security_group_ids  = [aws_security_group.vpc_endpoints.id]
  private_dns_enabled = true
  tags = merge(local.common_tags, { Name = "${var.project_name}-vpce-logs" })
}

# Gateway endpoint — free; routes S3 traffic (ECR image layers) via private path
